...
const SignIn = () => {
  ...
  const fieldData: any = {
    userName: {
      label: " Username/Email address/Mobile",
      name: "userName",
      ...
      await ApiUtil.postWithoutToken(authEndPoints.login, {
        ...
        .then(async (res: any) => {
          ...
          await getUserDetails().then((res: any) => {
            ...
            if (res.data.permission.length < 1) {
              dispatch(
                setSnackBarFailed({
                  snackBarMessage: `${res?.data?.name} ${loginPageConst.NO_ACCESS}`,
                })
              );
              setLoader(false);
            } else {
              setTimeout(() => {
                const permissionList = res.data.permission;

                if (permissionList.includes("Restaurant")) {
                  navigate(`/clinical/appointment`);
                  // navigate(RouteUrls.restaurantUrl);
                } else if (permissionList.includes("Medical Camp")) {
                  navigate(RouteUrls.medicalCampUrl);
                } else if (permissionList.includes("Clinic")) {
                  navigate(`/clinical/appointment`);
                } else if (permissionList.includes("Spiritual Tour")) {
                  navigate(RouteUrls.spiritualTourUrl);
                } else {
                  navigate(RouteUrls.customerRegistrationUrl);
                }
                ...
                // console.log("mnmnmn settimeout run");
              }, 2000);
            }
          });
        })
      ...
      >
        <Grid
          sx={{
            width: "100%",
            height: "100vh",
            backgroundColor: "#A7B59B",
            position: "relative",
          }}
        >
          <Grid sx={{ position: "absolute", bottom: 0, right: 0 }}>
            <LogInIcon />
          </Grid>
        </Grid>
      </Grid>
      ...
      >
        <Typography variant="h3" color="#8A8A8A" fontWeight={400}>
          ...
          <a
            ...
            style={{ textDecoration: "none", color: "#597F59" }}
          >
            Techbumbles
          </a>
        </Typography>
